  ...
  function buildTable() {
    ...
    var pickaday = document.getElementById("pickaday");
    ...
    for (var i = 0; i < length; i++) {
      if (daysel == dayid[i] && (csrsel == csrid[i] || (usualcsr.includes(csrid[i]) && csrsel == altcsr[usualcsr.indexOf(csrid[i])]))) {
        ...
        cellText = document.createTextNode(shipto[i].replace(/~/g, " "));
        cell.appendChild(cellText);
        row.appendChild(cell);
        cell = document.createElement("td");
        cellText = document.createTextNode(dept[i].replace(/~/g, " "));
        cell.appendChild(cellText);
        row.appendChild(cell);
        cell = document.createElement("td");
        cellText = document.createTextNode(phone[i].replace(/~/g, " "));
        cell.appendChild(cellText);
        row.appendChild(cell);
        cell = document.createElement("td");
        cellText = document.createTextNode(manager[i].replace(/~/g, " "));
        cell.appendChild(cellText);
        row.appendChild(cell);
        cell = document.createElement("td");
        var link = document.createElement('a');
        var linkText = document.createTextNode(called[i]);
        link.appendChild(linkText);
        link.setAttribute("href","/calllists/list_update?id="+call_list_id[i]);
  ...
  //   //******* build a call list for a route/CSR combination
  //   $('#listtab').remove();
  //   var csr = document.getElementById("csrid").innerHTML;
  //   var csrid = getData(csr);
  //   var route = document.getElementById("routeid").innerHTML;
  //   var routeid = getData(route);
  //   var cust = document.getElementById("customer").innerHTML;
  //   var customer = getData(cust);
  //   var ship = document.getElementById("shipto").innerHTML;
  //   var shipto = getData(ship);
  ...
  //   var pickacsr = document.getElementById("pickacsr");
  //   var csrsel = pickacsr.options[pickacsr.selectedIndex].text;
  //   var routesel = pickaroute.options[pickaroute.selectedIndex].text;
  //   var listtable = document.getElementById('listtable');
  //   var tbl = document.createElement('table');
  //   tbl.setAttribute("id","listtab");
  //   var row = document.createElement('tr');
  ...
  //   var length = customer.length;
  //   for (var i = 0; i < length; i++) {
  //     if (csrsel == csrid[i] && routesel == routeid[i]) {
  //       var row = document.createElement("tr");
  //       var cell = document.createElement("td");
